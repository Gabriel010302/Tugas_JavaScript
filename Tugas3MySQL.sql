...
SELECT COUNT(kode) AS total_produk FROM produk WHERE kode LIKE "K%";
...
SELECT nama_pelanggan, jk, kartu_id as kartu FROM pelanggan WHERE kartu_id in (SELECT id FROM kartu WHERE diskon > 0.03);
